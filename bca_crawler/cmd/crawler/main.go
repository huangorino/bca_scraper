...
func main() {
	...
	if err != nil {
		log.Fatalf("❌ Failed to load start page: %v", err)
		return
	}
	...
	if maxID == 0 {
		log.Warn("⚠️ No announcements found. Exiting.")
		return
	}

	log.Infof("✅ Parsed announcements. Max ann_id: %d", maxID)

	for i := 3599525; i <= maxID; i++ {
		...
		if err != nil {
			log.Errorf("❌ Failed to load ID %d: %v", i, err)
			continue
		}
		...
		if err := db.SaveAnnouncement(database, a); err != nil {
			log.Errorf("❌ Failed to save ID %d: %v", i, err)
		} else {
